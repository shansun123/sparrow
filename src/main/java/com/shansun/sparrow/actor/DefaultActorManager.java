...
import com.shansun.sparrow.actor.MessageWrapper.SpreadMode;
...
public class DefaultActorManager implements ActorManager {
	...
	int									threadCount				= Constants.DEFAULT_ACTOR_THREAD_COUNT;

	/** 实际使用线程计数 */
	int									realUsedThreadCount		= 0;

	/** 每条线程运行接受最大消息数（队列中） ，如果为0，则表示没有限制 */
	long								maximumMessageSize		= 0;

	/**  */
	RejectedMessageHandler				rejectedMessageHandler	= RejectedMessageHandlers.discardPolicy();
	...
	AbstractActor						noneMessageActor		= new NoneMessageActor();
	{
	...
	public void terminate() {
		...
		for (ThreadWrapper th : threads) {
			...
			if (runnable instanceof MessageRedcapRunnable) {
				((MessageRedcapRunnable) runnable).running = false;
			} else {
				th.interrupt();
			}
		}
	...
	public int broadcast(Message message, Actor from) {
		...
		if (from == null) {
			wrapper.setSource("unknown-actor");
		} else {
			wrapper.setSource(from.getName());
		}
	...
	public int send(Message message, Actor sourceActor, String targetName) {
		...
		if (sourceActor == null) {
			...
			wrapper.setSource(sourceActor.getName());
		}
		wrapper.setTarget(targetName);
